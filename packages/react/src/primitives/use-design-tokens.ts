 ...
 *
 * Web-specific: elevation returns CSS boxShadow strings.
 */
...
export interface DesignTokensWeb {
  colors: {
  ...
  elevation: { none: string; sm: string; md: string; lg: string; xl: string };
  motion: {
  ...
  identity: {
    ...
    // Phase 4 / plan 3 Task 21 — LayerBackground replaces legacy BackgroundConfig.
  ...
  /** Resolve borderRadius with radiusPattern applied */
...
function elevToCSS(e: { shadowOffset: [number, number]; shadowRadius: number; shadowOpacity: number; shadowColor: string }): string {
...
export function useDesignTokens(tokens: Record<string, unknown> | undefined): DesignTokensWeb {
  ...
  const identityConfig: IdentityConfig = {
    ...
    gradients: {
      ...
      cards: get<boolean | 'vibrant' | 'soft' | 'muted'>(tokens, 'identity.gradients.cards', false),
